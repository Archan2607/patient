...
class FeatureExtractor:
    ...
    def plank_3_left_right_std(self, x):

        '''
        Function calculates the standard deviation of left sensors of plank 3 in terms of
        standard deviation of right sensors of plank 3
        '''

        left_sensors = ['LC10', 'LC11']
        left_sensors_pct = [x+'_pct' for x in left_sensors]
        right_sensors = ['LC9', 'LC12']
        right_sensors_pct = [x+'_pct' for x in right_sensors]
        try:
            left_std_val = np.round(np.std(x[left_sensors_pct].values, ddof = 1), 2)
        except:
            left_std_val = 0
        finally:
            pass
        try:
            right_std_val = np.round(np.std(x[right_sensors_pct].values, ddof = 1), 2)
        except:
            right_std_val = 0
        finally:
            pass
        try:
            plank_3_left_right_std_val = np.round(left_std_val/right_std_val, 2)
        except:
            plank_3_left_right_std_val = left_std_val
        finally:
            return plank_3_left_right_std_val
